...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
    new_adventurer(state,handPos);
    return 0;
			
    case council_room:
     new_council_room(state,handPos);
      return 0;
    ...
    case smithy:
    new_smithy(state,handPos);
      return 0;
		
    case village:
    new_village(state,handPos);
      return 0;
...
int new_smithy(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
  int i;
  //+3 Cards
...
int new_adventurer(struct gameState *state)
{
  int currentPlayer = whoseTurn(state);
  int drawntreasure=0;
  int cardDrawn;
...
int new_council_room(int handPos, struct gameState *state)
{
	int i;
	int currentPlayer = whoseTurn(state);
	for (i = 0; i < 4; i++){
...
int new_village(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
  ...
  state->numActions = state->numActions + 2;    // This line contains the change for Bug 3.
...
int new_great_hall(struct gameState *state, int handPos)
{
  //+1 Card
  drawCard(currentPlayer, state);
