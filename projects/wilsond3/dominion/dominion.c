...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState* state, int handPos, int* bonus)
{
   ...
   int tributeRevealedCards[2] = {-1, -1};
      // moved above the if statement
   int temphand[MAX_HAND];
   int drawntreasure = 0;
   int cardDrawn;
      // counter for the temp hand
   int z = 0;
   ...
   switch (card)
   {
      ...
      case council_room:
         return _council_room(state, currentPlayer, handPos);
      case feast:
         return _feast(state, currentPlayer, choice1);
      case gardens:
         return -1;
      case mine:
      ...
      case cutpurse:
         return _cutpurse(state, currentPlayer, handPos);
      ...
      case treasure_map:
         ...
         //no second treasure_map found in hand (wilsond3: THIS SEEMS TO BE A PROBLEM)
      return -1;
    }

   return -1; // (wilsond3: THIS SEEMS TO BE A PROBLEM)
}
