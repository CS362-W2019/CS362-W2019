...
int main()
{
    ...
    for (int i=0; i<testCount; i++) {
        ...
        int result = cardEffect(village, 0, 0, 0, &g, handPos, NULL);

        printf("TESTING village ITERATION %d:\n----------\n", i+1);

        int pass = 1;

        if (!assertEquals(result, 0, "village: Successfully returned with 0.")) {
            pass = 0;
        }
        if (!assertEquals(g.playedCardCount, oldPlayedCardCount + 1, "village: Played card pile contains 1 more card.")) {
            pass = 0;
        }
        if (!assertEquals(g.handCount[player], oldHandCount, "village: Player hand contains same number of cards.")) {
            pass = 0;
        }
        if (!assertEquals(g.numActions, oldNumActions + 2, "village: numActions increased by 2.")) {
            pass = 0;
        }
