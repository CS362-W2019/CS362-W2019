...
/***
* Assignment 2 Q3 - Code Refactor for 5 Cards
***/
...
// Adventurer Card
int playCardAdventurer(int drawntreasure, struct gameState *state, int currentPlayer, int temphand[], int z)
{
  int cardDrawn;
...
// Remodel Card
int playCardRemodel(struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos) 
{
    int i;
    int j;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      playCardAdventurer(drawntreasure, state, currentPlayer, &temphand[z], z);
      // while(drawntreasure<2) {
      //   if (state->deckCount[currentPlayer] <1) {//if the deck is empty we need to shuffle discard and add to deck
      //     shuffle(currentPlayer, state);
      //   }
    ...
    case remodel:
      playCardRemodel(state, currentPlayer, choice1, choice2, handPos);
      //     j = state->hand[currentPlayer][choice1];  //store card we will trash
    ...
    case smithy:
      playCardSmithy(state, currentPlayer, handPos);
      // //+3 Cards
      // for (i = 0; i < 3; i++)
      // {
      //   drawCard(currentPlayer, state);
    ...
    case village:
      playCardVillage(state, currentPlayer, handPos);
      // //+1 Card
      // drawCard(currentPlayer, state);
