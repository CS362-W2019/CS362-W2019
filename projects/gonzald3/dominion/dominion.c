...
int adventurerCard(struct gameState *state){
	...
	while(drawntreasure<2){
		...
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
		  ...
		  /***************
		  Bug removed incrementing of the counter for the temp hand
		  //z++;
		  ***************/
		}
		  }
...
//Refactored Function2
int smithyCard(struct gameState *state, int handPos){
	int i;
	int currentPlayer = whoseTurn(state);
	//+3 Cards
	/************Introduce a larger amount of draws more than 3************/
      for (i = 0; i < 10; i++)
	{
	  drawCard(currentPlayer, state);
	}
...
int council_roomCard(struct gameState *state, int handPos){
		 ...
		 for (i = 0; i < 4; i++)
		{
		  ...
		  /*********Introduce bug ***************/
		  /* remove i < state->numPlayers ****/
		  for (i = 0; i < 0; i++)
		{
		  /****According to the rules this should be i != currentPlayer**********/
		  if ( i == currentPlayer ) 
			{
			  drawCard(i, state);
			}
...
int feastCard(struct gameState *state, int choice1){
	...
	/**************Introduced a bug to remove ability to gain card costing 5*******/
	/*
