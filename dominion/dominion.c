...
int ref_adventurer(struct gameState *state, int handPos, int *bonus, int drawntreasure)
{
    int currentPlayer = whoseTurn(state);
...
int ref_smithy(struct gameState *state, int handPos, int *bonus)
{
    int i;
    int currentPlayer = whoseTurn(state);
...
int ref_gardens(struct gameState *state)
{
...
int ref_village(struct gameState *state, int handPos, int *bonus)
{
...
int ref_councilroom(struct gameState *state, int handPos, int *bonus)
{
    int i;
    int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
        ref_adventurer(state, handPos, bonus, drawntreasure);
/*      while(drawntreasure<2){
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
	  shuffle(currentPlayer, state);
	}
...
*/
    ...
    case council_room:
        ref_councilroom(state, handPos, bonus);
/*      //+4 Cards
      for (i = 0; i < 4; i++)
	{
	  drawCard(currentPlayer, state);
...
*/
    ...
    case smithy:
        ref_smithy(state, handPos, bonus);
/*      //+3 Cards
      for (i = 0; i < 3; i++)
	{
	  drawCard(currentPlayer, state);
...
*/		
    case village:
        ref_village(state, handPos, bonus);
/*      //+1 Card
      drawCard(currentPlayer, state);
